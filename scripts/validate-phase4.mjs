...
const required=[
  ...
  'src/scripts/archive-controller.ts','src/scripts/catalogue-search.ts','src/lib/archive-state.ts','src/lib/search-core.ts',
  'tests/e2e/phase4.spec.ts','tests/unit/search-core.test.ts','tests/unit/archive-state.test.ts','tests/fixtures/catalogue-250.json'
];
...
const archiveControls=fs.readFileSync(path.join(root,'src/components/archive/ArchiveControls.astro'),'utf8');
const controller=fs.readFileSync(path.join(root,'src/scripts/archive-controller.ts'),'utf8');
const searchController=fs.readFileSync(path.join(root,'src/scripts/catalogue-search.ts'),'utf8');
...
const checks=[
  ...
  [search.includes('data-search-suggestion')&&search.includes('All projects'), 'Project Search must expose visible discovery suggestions and an all-projects route.'],
  [searchController.includes('featuredResults')&&searchController.includes('searchCatalogue'), 'Project Search must show featured defaults and searchable ranked results.'],
  [archiveControls.includes('data-archive-clear')&&controller.includes('resetFilters'), 'Project browser must expose an explicit filter reset.'],
  [!search.includes('Random project')&&!archiveControls.includes('Random project'), 'Random-project novelty controls must not return.'],
