...
const required = [
  ...
  'public/projects/pdf-studio/screenshot-desktop.png',
  'public/projects/pdf-studio/screenshot-workspace.png',
];
...
for(const [slug,[tier,type,component,duration,scene,animation]] of Object.entries(reconstructed)){
...
const pdfChecks=[
  [pdf.includes('tier: P5'),'pdf-studio must remain P5.'],
  [pdf.includes('type: static'),'pdf-studio preview type must be static.'],
  [pdf.includes('provenance: captured'),'pdf-studio preview must be labelled captured.'],
  [pdf.includes('poster: /projects/pdf-studio/screenshot-desktop.png'),'pdf-studio captured poster contract missing.'],
  [pdf.includes('source: /projects/pdf-studio/screenshot-workspace.png'),'pdf-studio real workspace gallery proof missing.'],
];
for(const [ok,msg] of pdfChecks){if(!ok){console.error(msg);process.exit(1);}}
...
console.log(`Phase 7 preview validation passed (PDF Studio captured proof + 6 reconstructed previews, ${(size/1024).toFixed(1)} KB WORDSTRIKE WebM).`);
