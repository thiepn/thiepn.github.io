...
const hasIndexField=['src/components/index/StaticIndexField.astro','src/components/index/LivingIndexField.astro'].some((file)=>fs.existsSync(path.join(root,file)));
if(!hasIndexField)fail.push('missing static/Living Index field implementation');
const read=(f)=>fs.readFileSync(path.join(root,f),'utf8');
const home=read('src/pages/index.astro');
for(const marker of ['00','Featured Work','Browse by Type','Project Archive','Collections','Recent Activity']){if(!home.includes(marker))fail.push(`homepage missing ${marker}`)}
...
for(const marker of ['Index','Projects','Collections']){if(!header.includes(marker))fail.push(`header missing ${marker}`)}
...
console.log(`Phase 2 static design validation passed (${required.length} required files).`);
